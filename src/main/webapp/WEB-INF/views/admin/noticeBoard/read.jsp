...
<html>
<head>
<title>LISTENER 공지사항 상세보기</title>
<meta charset="utf-8" />
<meta name="viewport"
				...
				<div class="box box-primary">
                  ...
                  <div class="form-group">
                     <label for="exampleInputEmail1">닉네임: ${login.nickname}</label>
						...
						&nbsp;&nbsp;
						<div class="form-group">
							<label for="exampleInputPassword1">말머리</label> <c:if test="${ 1 eq  noticeBoardVO.mustRead}">
									 <input type="text"
								name='mustRead' value="필독" class="form-control"
								readonly="readonly">
								</c:if>
								<c:if test="${ 1 ne  noticeBoardVO.mustRead}">
									<input type="text"
								name='mustRead' value="일반" class="form-control"
								readonly="readonly">
								</c:if>
						</div>
...
<script>
	$(document).ready(function() {
		...
		$(".btn-warning").on("click", function() {
			formObj.attr("action", "/admin/noticeBoard/modify");
			formObj.attr("method", "get");
			formObj.submit();
		});

		$(".btn-danger").on("click", function() {
			formObj.attr("action", "/admin/noticeBoard/remove");
			formObj.submit();
		});

		$(".btn-primary").on("click", function() {
			formObj.attr("method", "get");
			formObj.attr("action", "/admin/noticeBoard/list");
			formObj.submit();
		});
