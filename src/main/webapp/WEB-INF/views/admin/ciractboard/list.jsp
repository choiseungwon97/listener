...
<html>
<head>
<title>LISTENER 동아리활동 게시판</title>
<meta charset="utf-8" />
<meta name="viewport"
				...
				<table class="alt">
					...
					<tbody>
						<c:if test="${!empty list}">
							<c:forEach items="${list}" var="ciractBoardVO" varStatus="status">

								<tr>
									<td>${status.count}</td>
									<td><a
										href='/admin/ciractboard/read${pageMaker.makeSearch(pageMaker.cri.page) }&cabNo=${ciractBoardVO.cabNo}'>
											${ciractBoardVO.title} </a></td>
									<td>${ciractBoardVO.nickname}</td>
									<td><fmt:formatDate pattern="yyyy-MM-dd HH:mm"
											value="${ciractBoardVO.regdate}" /></td>
									<td><span class="badge bg-red">${ciractBoardVO.viewcnt }</span></td>
								</tr>
