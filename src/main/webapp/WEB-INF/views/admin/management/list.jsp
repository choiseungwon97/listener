					...
					<div class="content">
                     ...
                     <select name="searchType">
                        ...
                        <option value="k"
                           ...
                           닉네임</option>
                        <option value="ink"
                           <c:out value="${cri.searchType eq 'ink'?'selected':''}"/>>
                           모두</option>
                     </select>
                  </div>
							<div class='col-3 col-12-small'>
						...
						<c:if test="${!empty list}">
							<c:forEach items="${list}" var="memberVO" varStatus="status">
								...
								<tr>
									<td>${status.count}</td>
									<td><a
										href='/admin/management/readPage${pageMaker.makeSearch(pageMaker.cri.page) }&id=${memberVO.id}'>
											${memberVO.id} </a></td>
									<td>${memberVO.name}</td>
									<td>${memberVO.nickname}</td>
									<td>${memberVO.stid}</td>
									<td>${memberVO.memberSession}</td>
